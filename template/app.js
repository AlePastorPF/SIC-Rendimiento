...
function attachSWC(data){
  ...
  Object.keys(byPlayer).forEach(p=>{
    const arr = byPlayer[p].sort((a,b)=> parseLocalDate(a.Fecha) - parseLocalDate(b.Fecha));
...
/* Convierte un string 'YYYY-MM-DD' (o un Date ya existente) en un Date de
   medianoche en la ZONA HORARIA LOCAL del navegador. Usar SIEMPRE esta
   funcion (y nunca "new Date('YYYY-MM-DD')" directo) para leer las fechas
   del dataset: "new Date('YYYY-MM-DD')" las interpreta como UTC, lo que en
   Argentina (UTC-3) hace que se muestren un dia antes del real. */
...
function renderProximoPartido(){
  ...
  const proximos = SIC_FIXTURE_2026
    .filter(p => parseLocalDate(p.fecha) >= hoy)
...
function dayIndexToDate(idx){
  const utc = new Date(idx*86400000);
...
function renderTimelineMarkers(scope){
  ...
  const seen = new Set();
  const uniqueActs = [];
  scope.slice().sort((a,b)=> parseLocalDate(a.Fecha)-parseLocalDate(b.Fecha)).forEach(r=>{
...
function renderKpiCards(filtered, f, fullDataInSeason){
  ...
  if(f.jugador.length!==1){
    ...
    const sorted = filtered.slice().sort((a,b)=> parseLocalDate(b.Fecha)-parseLocalDate(a.Fecha));
    const last = sorted[0];
